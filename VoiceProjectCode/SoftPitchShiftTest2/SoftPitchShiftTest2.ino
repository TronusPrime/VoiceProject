...
// Filter and Pitch Shift
FilteredStream<int32_t, float> filtered(i2s_mic, info.channels); // Apply FIR filter on input
PitchShiftOutput<int32_t, VariableSpeedRingBuffer<int32_t>> pitchShift(i2s_out); // Apply pitch shift on filtered audio

StreamCopy copier(pitchShift, filtered); // Copies filtered and pitch-shifted audio from mic to DAC
...
void setup(void) {
  ...
  // Configure filters for all channels
  ...
  Serial.println("I2S microphone, filter, pitch shift, and DAC started...");
}

// Arduino loop - continuously copy audio from mic to DAC with filtering and pitch shifting
void loop() {
  copier.copy();  // Copy filtered and pitch-shifted audio
}
